...
import com.cobaltumapps.simplecalculator.ui.activities.extra.ArchiveActivity
import com.cobaltumapps.simplecalculator.ui.activities.extra.NewsUpdateActivity
import com.cobaltumapps.simplecalculator.ui.activities.extra.PurchasesActivity
import com.cobaltumapps.simplecalculator.ui.activities.extra.SettingsActivity
...
class MainCalculatorActivity : AppCompatActivity(), NavigationView.OnNavigationItemSelectedListener,
    ...
    private val sharedPreferences by lazy {
        getSharedPreferences(ConstantsCalculator.vaultPreferences, MODE_PRIVATE)
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        VibrationSingleton.getInstance(getSystemService(VIBRATOR_SERVICE) as Vibrator)
        MemoryStorageControllerSingleton.getInstance()

        actionBarDrawerToggle = ActionBarDrawerToggle(
            this@MainCalculatorActivity,
            binding.calculatorDrawer,
            null,
        ...
        val calculatorPageFragment = supportFragmentManager.findFragmentByTag(CalculatorPageFragment.Companion.FRAG_TAG) as? CalculatorPageFragment
        if (calculatorPageFragment == null) {
            val newFragment = CalculatorPageFragment(this)
            supportFragmentManager.commit {
                replace(binding.calculatorPlaceHolder.id, newFragment, CalculatorPageFragment.Companion.FRAG_TAG)
            }
        }
        else
    ...
    override fun onNavigationItemSelected(item: MenuItem): Boolean {
        when(item.itemId) {
            R.id.menu_nav_converters -> startActivity(
                Intent(
                    this@MainCalculatorActivity,
                    ExtraCalculatorsActivity::class.java
                )
            )
            R.id.menu_nav_features -> startActivity(
                Intent(
                    this@MainCalculatorActivity,
                    PurchasesActivity::class.java
                )
            )
            R.id.menu_nav_archive -> startActivity(
                Intent(
                    this@MainCalculatorActivity,
                    ArchiveActivity::class.java
                )
            )
            R.id.menu_nav_news_update -> startActivity(
                Intent(
                    this@MainCalculatorActivity,
                    NewsUpdateActivity::class.java
                )
            )
            R.id.menu_nav_introduction -> startActivity(
                Intent(
                    this@MainCalculatorActivity,
